...
def get_team_members_full():
    rows = execute_query('''
        SELECT position, last_name, first_name, middle_name, photo_path, description, telegram, email, pulse
        FROM team
        ORDER BY id ASC
    ''', fetch=True) or []
    members = []
    for r in rows:
        position, last_name, first_name, middle_name, photo_path, description, telegram, email, pulse = r
        contacts = []
        if telegram:
...
async def handle_task_click(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        filtered_comments = [comment for comment in comments if comment["author"] != "WALL-E [robot]"]
...
async def process_comment(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        asyncio.create_task(delete_after_delay(message.chat.id, progress_message.message_id, 3))
        ...
        await progress_message.edit_text(f"❌ Ошибка: {str(e)}")
        asyncio.create_task(delete_after_delay(message.chat.id, progress_message.message_id, 5))
    finally:
        await state.clear()
