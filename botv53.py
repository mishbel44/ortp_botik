    ...
    async def get_issue_comments(self, issue_key):
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                return [comment["body"] for comment in comments] if comments else []
...
async def handle_task_click(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        last_comment = comments[-1] if comments else "Нет комментариев"
        ...
        message_text = (
            ...
            f"💬 Последний комментарий: <b>{last_comment}</b>\n\n"
    ...
    try:
        ...
        if event == 'status_changed':
            ...
            if should_notify():
                ...
                sent_message = await bot.send_message(chat_id=user_id, text=message_text, parse_mode="HTML")
                execute_query(
                ...
                execute_query('''
                    ...
                    AND keep.id IS NULL
                ''', (user_id, user_id))
                asyncio.create_task(delete_after_delay(user_id, sent_message.message_id, 5))
